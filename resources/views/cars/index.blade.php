    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <x-nav-link :href="route('car.create')" type="submit"
                ...
                @foreach($cars as $car)
                    <div class="p-8 flex flex-col gap-8">
                        <div class='flex justify-between gap-x-2'>
                            <p class="w-full font-bold text-base">{{ $car->number }}</p>
                            <p class="w-full font-bold text-base">{{ $car->number }}</p>
                            <p class="w-full font-normal text-base">{{ $car->mark }}</p>
                            <p class="w-full font-bold text-base">{{ $car->model }}</p>
                        </div>
                    </div>
                @endforeach
